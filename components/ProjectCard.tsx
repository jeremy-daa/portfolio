...
const ProjectCard = ({ title, description, image, link }: ProjectCardProps) => {
  return (
    <div className="flex flex-col gap-2 justify-center items-center">
      <div className="flex flex-col justify-center items-center relative w-[400px] h-[188px] rounded-lg overflow-hidden duration-300 boxshadow projectcard">
        <Image
          src={image}
          alt="project"
          width={700}
        ...
        <div className="absolute top-0 left-0 w-full backdrop-blur-sm h-full z-10 translate-y-[100%] duration-300 p-5 flex flex-col items-center justify-center gap-4 text-center bg-[var(--black-transparent)] coolborder-parent">
          <h1 className="text-[var(--white)] text-2xl tracking-wider font-bold z-20 w-fit text-center coolborder p-2">
            {title}
          </h1>
          {description?.map((desc, index) => (
            <p key={index} className="text-[var(--white)] text-base z-20 w-fit">
              {desc}
            </p>
          ))}
          <Link
